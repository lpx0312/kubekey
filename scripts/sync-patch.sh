...
if git cherry-pick "$PATCH_RANGE"; then
  ...
  echo "       - 证书续期修复 (renew_script.sh): 不得出现 {{- if <v1.20.0 }} 死代码;"
  echo "         getCertValidDays 应解析 RESIDUAL TIME 列; k8s-certs-renew.service"
  echo "         的 ExecStart 应为 /usr/local/bin/kube-scripts/renew_script.sh"
  echo "    2. 标记解决并继续:"
