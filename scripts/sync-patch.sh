...
if git cherry-pick "$PATCH_RANGE"; then
  ...
  echo "       - Harbor HA 修复 (harbor.yml + image-registry/meta/main.yaml + harbor/tasks/install.yaml, 补丁10):"
  echo "         harbor.yml 的 hostname 条件必须是 .image_registry.auth.registry | empty"
  echo "         (用 registry 域名, 不能是 .groups.image_registry | len | lt 1);"
  echo "         keepalived 两处 when 必须是单一的 .image_registry.ha_vip | empty | not"
  echo "         (不能用 len|lt 1 也不能用 len|gt 1; gt 1 会让 keepalived 被跳过, VIP 起不来)"
  echo "    2. 标记解决并继续:"
